...
export default function ChartBubble({ chart }: { chart: any }) {
  if (!chart) return <div>No chart data</div>;

  // If backend returned a message (no chart), show it instead of an empty canvas
  ...
  // debug: inspect incoming chart payload and computed labels/datasets
  try { console.log('[AgoraLearn] ChartBubble chart prop:', chart); } catch {}
  ...
  // Normalize different chart payload shapes into a `series` array of { name, points: [{x,y}] }
  let series: Series[] = [];
  ...
  // fallback: accept table-like payloads { columns: [], rows: [] } either at root or under data
  ...
  if (table) {
    ...
    // choose first column as x, remaining numeric columns as series
    const xCol = 0;
  ...
  // Build labels from first series' x values
  const labels = series.length > 0 ? series[0].points.map((p) => String(p.x)) : [];
  // Ensure numeric y values and filter NaNs
  ...
  try { console.log('[AgoraLearn] ChartBubble labels:', labels, 'datasets length:', datasets.length); } catch {}
